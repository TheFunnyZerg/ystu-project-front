...
export const useTableState = () => {
  ...
  const calculateTotalCredits = () => {
    return rows.reduce((total, row) => {
      return total + row.data.reduce((rowTotal, cell) => {
        return rowTotal + cell.reduce((cellTotal, discipline) =>
            cellTotal + (discipline?.credits || 0), 0);
      }, 0);
    }, 0);
  };
